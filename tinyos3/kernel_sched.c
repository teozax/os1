...
rlnode SCHED[10];         	  /* The scheduler queues */
...
/*
  ...
  *** MUST BE CALLED WITH sched_spinlock HELD ***
*/
static void sched_queue_add(TCB* tcb, Thread_state tcb_st, int tcb_prior)
{
  rlist_push_back(& SCHED[tcb_prior-1], & tcb->sched_node);
...
static void sched_make_ready(TCB* tcb)
{
	...
	if(tcb->phase == CTX_CLEAN)
		sched_queue_add(tcb, tcb->state, tcb->priority);
...
static TCB* sched_queue_select()
{
  ...
  //find the first NON empty SCHED list
  while(is_rlist_empty(&SCHED[j]) && j<NumScheds-1)
  	j++;

  /* Get the head of the SCHED list */
  rlnode *sel = rlist_pop_front(& SCHED[j]);
...
void yield(enum SCHED_CAUSE cause)
{
  ...
  switch(cause)
  {
    ...
    case SCHED_QUANTUM:
    	current->priority = (current->priority<NumScheds) ? current->priority+1 : NumScheds;


    case SCHED_MUTEX:
    	current->mutex_counter ++;
    	if(current->mutex_counter==10)
    	{
    		current->priority = (current->priority<NumScheds) ? current->priority+1 : NumScheds;
    		current->mutex_counter=0;
    	}
  ...
  {
    current->aging=0;
    current->priority=current->prev1;
  }


  if(priority_aging == 3000)
  {
    l = (l<NumScheds-1) ? l+1 : 1;
    while(!is_rlist_empty(& SCHED[l]))
    {
      rlnode *sel = rlist_pop_front(& SCHED[l]);
      sel->tcb->prev1=sel->tcb->priority;
      sel->tcb->priority --;
      sel->tcb->aging = 1;
      rlist_push_back(& SCHED[l-1], sel);
    }
    priority_aging=0;
  }
...
void gain(int preempt)
{
  ...
  if(current != prev) {
    ...
    {
      case READY:
        if(prev->type != IDLE_THREAD) sched_queue_add(prev, prev->state, prev->priority);
        break;
      case EXITED:
      	release_TCB(prev);
        break;
...
void run_scheduler()
{
  ...
  //curcore->idle_thread.priority = 1;
